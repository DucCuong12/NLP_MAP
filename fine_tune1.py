...
print(os.getcwd())
# model = model.to("cuda:1")  # Chuyển model sang cuda:1 trước
...
def fine_tune(model, train_dataloader, arg, epochs = 3):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 20 == 0:
            if arg.loc:
                torch.save(model.state_dict(), f"checkpoints/new_loss_{epoch+20}.pth")
                print(f"Saved best model at epoch {epoch+1} with loss {total_loss:.4f}")
            else:
                torch.save(model.state_dict(), f"checkpoints/continue_loss_{epoch+60}.pth")
                print(f"Saved best model at epoch {epoch+61} checkpoints/continue_loss_{epoch}.pth with loss {total_loss:.4f}")
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        if args.pro:
            ...
            print("Continue training from checkpoints/continue_loss_40.pth")
            state_dict = torch.load('checkpoints/continue_loss_40.pth')
            model.load_state_dict(state_dict)  # Use strict=False to ignore missing keys
